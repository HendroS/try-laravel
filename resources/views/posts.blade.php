...
{{-- @if (request('category'))
<h1 class="mb-3 text-center">{{ request('category') }}</h1>
@endif
@if (request('author'))
<h1 class="mb-3 text-center">{{ request('author') }}</h1>   
@endif --}}
...
<div class="card mb-3">
    @if ($posts[0]->image)
        ...
        <img src="{{ asset('storage/'.$posts[0]->image ) }}"
            class="card-img-bottom img-fluid" alt="{{ $posts[0]->category->name }}">
        </div>    
    @else    
...
<div class="container">
    <div class="row">
        ...
        @if ($post->image)
         ...
         @endif
        <div class="card-body">
            <h5 class="card-title"><a class="text-decoration-none" href="/posts/{{ $post->slug }}">{{ $post->title }}
                    </a></h5>
            <p>
                <small class="text-muted">
